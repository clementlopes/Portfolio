    ...
    <div class=" grid grid-cols-1 lg:grid-cols-3 gap-6 ">
      ...
      <div class="w-full col-span-2 shadow-xl rounded-xl border border-base-300">
        ...
        <div class="flex w-full justify-end gap-2 p-4">
          <button @click="cancelChanges()" class="btn btn-outline btn-error rounded-md">Cancel</button>
          <button @click="saveChanges()" class="btn btn-outline btn-success rounded-md">Save Changes</button>
        </div>
      </div>
    </div>
...
const saveChanges = async () => {
  ...
  if (newPassword.value || confirmPassword.value || currentPassword.value) {
    ...
    if (passwordResponse) {
      toastStore.openToast({type: 'success', message: 'Password changed successfully.'})
      cancelChanges();
      await navigateTo('/');

    } else {
      toastStore.openToast({type: 'error', message: 'Failed to change password!'})
      return
    }
